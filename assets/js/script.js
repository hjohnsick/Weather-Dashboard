...
var getSavedCities = function () {
  ...
  for (var i = 0; i < savedCities.length; i++) {
    $(".cities").append(
      `<button id="saved-city${i}" class="city-button" data-index="${i}">${savedCities[i]}</button>`
    );
    $(`#saved-city${[i]}`).css({
      "background-color": "#8972C8",
      color: "#FFFFFF",
      "margin-top": "7px",
      width: "100%",
    });
...
var displayCurrentWeather = function (data, cityName) {
  ...
  $("#todays-forecast").append(
    `<h3>${cityName} ${date} <img src="http://openweathermap.org/img/w/${icon}.png" alt="current weather"></h3>`
  );
  $("#todays-forecast")
    .append(
      `<ul class="weather-data"><li>Temp: ${currentTemp} &#8457</li><li>Wind: ${windSpeed} MPH</li><li>Humidity: ${humidity}%</li></ul>`
    )
    .css("border", "2px solid #40C3E0");
...
var displayFiveDayForecast = function (data) {
  ...
  for (var i = 0; i < 6; i++) {
    ...
    $(`#day-${i}`).append(
      `<ul class="day"><li>${date}</li><li><img src="http://openweathermap.org/img/w/${icon}.png" alt="current weather"></li><li>Temp: ${temp} &#8457</li><li>Wind: ${wind} MPH</li><li>Humidity: ${humidity}%</li></ul>`
    );
